...
*needs sentence about relationships*
...
plot(x.axis, y.axis, type='l', main="Norma distribution",
...
<span style="color: red;">Issue with this exercise: In task 4 we are to find the one that looks normally distributed. I think that the only one that looks normally distributed is Sepal Width, so it is our X in this case.</span>

<span style="color: red;">By plotting the scatterplot of X with i) petal length and ii) sepal length we cannot get the clusters required as there looks to be no linear relationship. See below the 2 plots, one with our actual X and the other one with Petal Width, the variable I assume must be used for this task.</span>

<span style="color: red;">"Put a threshold (limit) to isolate the data points of the first cluster corresponding to lower values of petal length and petal width" Why does he mention petal width? It is not normally distributed so it cannot be X and we are also not required to use it for the scatterplots: I think there s a mistake with this exercise!</span>

## *with X (sepal width)*
```{r}
plot(X, iris$Petal.Length, pch=19)
```
<span style="color: red;">to be honest this plot is ok because we can put a threshold of y<2.3 for example</span>
```{r}
plot(X, iris$Sepal.Length, pch=19)
```
<span style="color: red;">But on this one there is no way to cluster</span>
...
```
<span style="color: red;">Petal width seems to be much more suited for this exercise as it is easy to put a threshold and isolate</span>
<br/>
...
```{r}
margin_error <- qnorm(.95)*(X.sd/sqrt(length(X)))
ll_7_i <- X.mean - margin_error
ul_7_i <- X.mean + margin_error
```

The **lower limit is *`r ll_7_i`* ** and the **upper limit is *`r ul_7_i`* **
...
S.mean <- mean(S)
S.sd <- sd(S)
margin_error <- qnorm(.95)*(S.sd/sqrt(length(S)))
ll_7_ii <- S.mean - margin_error
ul_7_ii <- S.mean + margin_error
```
The **lower limit is *`r ll_7_ii`* ** and the **upper limit is *`r ul_7_ii`* **
...
<span style="color: red;">I am not good at explanations xD</span>
