...
sd(iris$Sepal.Width)
```
The data has a standard deviation of only 0.43cm. 
<br/>
<br/>
...
$$^{4}C_{2}=\frac{n!}{k!(n-k)!}=\frac{4!}{2!(4-2)!}=6$$
...
```
```{r}
percent(pnorm(3.5, mean=X.mean, sd=X.sd, lower.tail=TRUE) - 
...
unique(iris$Species[indices])
```
They all seem to belong to the same species setosa.
